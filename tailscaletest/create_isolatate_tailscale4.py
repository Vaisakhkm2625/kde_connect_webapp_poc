...
def setup_logging():
    logging.basicConfig(
        ...
        handlers=[
            ...
            logging.FileHandler("tailscale_debug.log", mode='w')
        ]
    )
    logging.info("Logging initialized. Writing to tailscale_debug.log")

setup_logging()

SERVER_PORT = 8080                      # port the HTTP server listens on
TAG = ["tag:kcadmin",f"tag:kcuser{ID}"]                     # ← change to your tag
...
def get_auth_key():
    ...
    logging.info(f"Generated auth key (ephemeral, tagged {TAG}): {auth_key[:12]}...")
    return auth_key
...
def setup_namespace_with_internet(ns_name: str):
    ...
    cmds = [
        ...
        ["sysctl", "-w", "net.ipv4.conf.all.rp_filter=2"],          # loose mode globally
        ["sysctl", "-w", f"net.ipv4.conf.{main_interface}.rp_filter=2"],  # specific to outbound iface
...
def start_http_server_in_namespace(ns_name: str, port: int = SERVER_PORT):
    ...
    """Runs a very simple HTTP server inside the namespace"""
    logging.info(f"Starting HTTP server in namespace {ns_name} on port {port}")
...
def cleanup_namespace(ns_name: str):
    ...
    """Cleanup namespace and related interfaces/rules"""
    logging.info(f"Cleaning up namespace {ns_name} ({ns_name[:6]})")
...
def enable_funnel_with_uuid(ns_name: str, socket_path: str):
    ...
    logging.info(f"Requesting public URL: funnel url")
    ...
    #result = subprocess.run(funnel_cmd, capture_output=True, text=True, stdout=sys.stdout, stderr=sys.stderr)
    ...
    import re
    # Extract URL from output like "https://theti5.tail25360.ts.net/"
    # Pattern looks for https:// followed by non-whitespace
    match = re.search(r'(https://[^\s]+)', result.stdout)
    if match:
        full_domain = match.group(1)
    else:
        # Fallback if parsing fails, though it shouldn't if funnel works
        logging.warning("Result URL not found in output, guessing...")
        full_domain = f"https://{ns_name}.{TAILNET}.ts.net" 
        # Note: simplistic guess, might correct the double ts.net if we cared, 
        # but the parsing is the primary fix.

    logging.info("\n" + "═"*70)
    logging.info(f"Public URL:  {full_domain}")
    logging.info("This URL is now publicly accessible on the internet")
    logging.info("═"*70 + "\n")
...
def main():
    ...
    try:
        ...
        logging.info("\n" + "="*70)
        logging.info(f"Success! Namespace '{ns_name}' is running")
        logging.info(f"→ HTTP server running inside on port {SERVER_PORT}")
        logging.info(f"→ Find Tailscale IP:")
        logging.info(f"  sudo ip netns exec {ns_name} tailscale ip -4")
        ...
        if args.cleanup:
            ...
            logging.info(f"  sudo ip netns delete {ns_name}")
            logging.info("  sudo iptables -t nat -D POSTROUTING -s 192.168.200.0/24 -j MASQUERADE")
